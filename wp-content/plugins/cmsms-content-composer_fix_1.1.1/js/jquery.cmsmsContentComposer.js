		...
		obj.methods = { 
			...
			buildButtons : function () { 
				if (obj.methods.butElems.find('> ul').length < 1) {
					var out = '<ul>' + 
						'<li>' + 
							'<a href="#" class="cmsms_row ' + cmsmsRow.icon + '" data-shortcode="cmsms_row">' + 
					...
					for (var key in cmsmsShortcodes) {
						out += '<li>' + 
							'<a href="#" class="' + key + ' ' + cmsmsShortcodes[key].icon + '" data-shortcode="' + key + '" data-pair="' + ((cmsmsShortcodes[key].pair) ? true : false) + '" data-multiple="' + ((cmsmsShortcodes[key].multiple) ? true : false) + '">' + 
